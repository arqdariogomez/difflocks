...
class DiffLocksInference():
    def __init__(self, path_ckpt_strandcodec, path_config_difflocks, path_ckpt_difflocks,
        ...
        # Guardamos en CPU por defecto, se mueve a GPU solo si se necesita en difusión
    ...
    def rgb2hair(self, rgb_img, out_path=None):
        ...
        try:
            ...
            # --- PREPARACIÓN DECODING (CPU SAFE) ---
            print("   [INFO] Transferring data to CPU...")
            scalp = scalp.cpu()
            torch.cuda.synchronize()
            self._clean(model); model=None
            
            # >>> FIX CLAVE: Mover diccionarios auxiliares a CPU <<<
            ...
            tbn_cpu = self.tbn_space_to_world.cpu()
            ...
            # Usamos las versiones CPU de todos los datos
            strands, _ = sample_strands_from_scalp_with_density(
                scalp[:,0:-1], density, codec, norm_dict_cpu, 
                mesh_data_cpu, tbn_cpu, self.nr_chunks_decode_strands)
